...
* TODO Thursday:
** Get mnist working
** give Dataset a __getitem__ method that can take ints or slices.
** In Hdf5Dataset, override __getitem__ to also take strings.
** let Hdf5Datasets also be a Dataset? In which case, change its name to Hdf5Dataset, and change HdfDatasetsSlice to Hdf5DatasetSlice.
